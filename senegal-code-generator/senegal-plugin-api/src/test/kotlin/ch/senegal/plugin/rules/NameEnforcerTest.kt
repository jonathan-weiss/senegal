...
internal class NameEnforcerTest {
    ...
    private fun assertThat(validity: Validity, name: String) {
        when(validity) {
            Validity.NAME_IS_VALID -> assertTrue(NameEnforcer.isValidName(name), "Name '$name' was assert to be valid, but was not.")
            Validity.NAME_IS_NOT_VALID -> assertFalse(NameEnforcer.isValidName(name), "Name '$name' was assert to be invalid, but was valid.")
        }
    }
