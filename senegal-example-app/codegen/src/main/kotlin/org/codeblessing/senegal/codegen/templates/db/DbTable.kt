...
data class DbTable(private val entity: EntityConcept) {
    ...
    private fun createDbField(entityField: EntityField): DbField {
        ...
        val kotlinModelField =
            KotlinModelField(entityField, kotlinModelClass)
        return when(entityField) {
            is PrimaryKeyFieldConcept -> PrimaryKeyDbField(
                entityField,
                dbTable,
                kotlinModelField
            )
            is ReferenceToPrimaryKeyFieldConcept -> ForeignKeyDbField(
                entityField,
                dbTable,
                kotlinModelField
            )
            is DataOnlyFieldConcept -> DataOnlyDbField(
                entityField,
                dbTable,
                kotlinModelField
            )
