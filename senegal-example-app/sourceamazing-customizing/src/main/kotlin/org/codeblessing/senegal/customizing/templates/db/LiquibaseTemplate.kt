...
object LiquibaseTemplate {
    ...
    fun createLiquibaseXmlIndexFileTemplate(dbTables: List<org.codeblessing.senegal.customizing.templates.db.DbTable>): String {
        return """
    ...
    fun createLiquibaseXmlFileTemplate(dbTable: org.codeblessing.senegal.customizing.templates.db.DbTable): String {
        return """
            <?xml version="1.0" encoding="UTF-8"?>
            <databaseChangeLog
                    xmlns="http://www.liquibase.org/xml/ns/dbchangelog"
            ...
            """
                        ...
                        <column name="${dbField.columnName}" type="${dbField.columnType}">
                            <constraints nullable="${ifElse(dbField.isMandatoryField, "false", "true")}" ${onlyIfIsInstance<org.codeblessing.senegal.customizing.templates.db.ForeignKeyDbField>(dbField) { referenceDbField -> "foreignKeyName=\"${referenceDbField.referenceName}\" references=\"${referenceDbField.referencedDbTable.tableName}(${referenceDbField.referencedDbTable.primaryKeyColumnName})\"" }} />
                        </column>
                            """ 
                        } }
                    </createTable>
